...
string test_graph = "test";
string partitioner = "hc2l0.3";
...
string query_result_file(string G){
    return experiments_folder + G + ".q.distance";
}
...
int main(int argc, char** argv){
    ...
    std::string order_file = order_path(g, partitioner);
    ...
    #ifdef CHECK
    std::string query_results_file = query_result_file(g);
    #endif
    ...
    #ifdef CHECK
    std::vector<unsigned> query_results = RoutingKit::load_vector<unsigned>(query_results_file);
    #endif
	...
	for (int i = 0; i < (int) query_sources.size(); ++i) {
        ...
        #ifdef CHECK
        unsigned ref_distance = query_results[i];
        #else
        unsigned ref_distance = -1;
        #endif
		query.reset().add_source(s).add_target(t).run();
        unsigned distance = query.get_distance();
        #ifdef CHECK
        string result = (ref_distance == distance) ? "pass" : "fail";
        #else
        string result = "NOCHECK";
        #endif
        cout << "Query Pair " << "(" << s << "," << t << ") cch distance " << distance << " reference " << ref_distance << " Test " << result << endl;
	}
